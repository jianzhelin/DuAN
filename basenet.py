...
class ResBase(nn.Module):
    def __init__(self,option = 'resnet18',pret=True):
        ...
        mod1 = mod[:4]
        mod2 = mod[4:]
        # print(mod[6]) # res4b bn22
        #self.model_ft =model_ft
        self.features1 = nn.Sequential(*mod1)
        self.features2 = nn.Sequential(*mod2)
        self.features = nn.Sequential(*mod)
    def forward(self, x):

        # x = self.features(x)
        temp_visual = self.features1(x)
        x = self.features2(temp_visual)

        x = x.view(x.size(0), self.dim)
        return x, temp_visual
...
class ResClassifier(nn.Module):
    def __init__(self, num_classes=9,num_layer = 2,num_unit=2048,prob=0.5,middle=1000):
        ...
        self.emb = nn.Sequential(*layers[0:8])
